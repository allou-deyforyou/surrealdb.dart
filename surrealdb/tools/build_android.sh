...
if [ "$1" = "x64" ]; then
  ...
  mkdir bin
  mv "target/x86_64-linux-android/release/libsurrealdb_core.so" "bin/libsurrealdb_core.so"
elif [ "$1" = "armv7" ]; then
  rustup target add armv7-linux-androideabi
  cargo build --target armv7-linux-androideabi --release
  mkdir bin
  mv "target/armv7-linux-androideabi/release/libsurrealdb_core.so" "bin/libsurrealdb.so"
else
  rustup target add aarch64-linux-android
  cargo build --target aarch64-linux-android --release
  mkdir bin
  mv "target/aarch64-linux-android/release/libsurrealdb_core.so" "bin/libsurrealdb_core.so"
fi
